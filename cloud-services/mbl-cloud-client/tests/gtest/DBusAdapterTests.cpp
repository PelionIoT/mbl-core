...
static void *SendReceiveRawMessage_MultiThread_reader_thread_start(void *mailbox)
{
    ...
    {
        if (mailbox_in_->receive_msg(output_msg, TI_DBUS_MAILBOX_MAX_WAIT_TIME_MS) != 0)
        {
...
static void *SendReceiveRawMessage_MultiThread_writer_thread_start(void *mailbox)
{
    ...
    for (char ch = 'A'; ch < 'Z' + 1; ++ch)
    {
        ...
        if (mailbox_in_->send_msg(input_message, TI_DBUS_MAILBOX_MAX_WAIT_TIME_MS) != 0)
        {
            pthread_exit((void *)-1);
        }
    }
...
static void *mbl_cloud_client_thread(void *adapter)
{
    ...
    {
        pthread_exit((void *)MblError::DBA_Temporary);
    }
...
{    
    ...
    for (size_t i = 0; i < sizeof(event_data.raw.bytes); i++){
        if (event_data.raw.bytes[i] != (uint8_t)(i*2)){
            result = MblError::DBA_Temporary;
            break;
        }
    }
...
TEST(SelfEvent, basic_no_adapter)
{
    ...
    for (size_t i = 0; i < sizeof(event_data.raw.bytes); i++){
        event_data.raw.bytes[i] = (uint8_t)(i*2);
    }
...
{    
    ...
    if (iter == SelfEventTest::random_numbers_.end()){
        sd_event_exit(ev->get_event_loop_handle(), (int)MblError::DBA_Temporary);
