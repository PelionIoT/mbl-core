...
int DBusAdapterImpl::sd_event_signal_handler_callback(sd_event_source* /*s*/,
                                             ...
                                             void* userdata)
{
    ...
    // If the handler function returns a negative error code, it will be disabled after 
...
{
    TR_DEBUG_ENTER;
    TR_DEBUG("Set signal handler for signal: %s", strsignal(signal));
    ...
    r = sd_event_add_signal(event_loop_handle_,
 	    nullptr,
 	    signal,
 	    sd_event_signal_handler_callback,
 	    this);
    if (r < 0) {
        TR_ERR("sd_event_add_signal for %s failed with error: %d - %s",
            strsignal(signal),
            r,
            strerror(-r)
        );
