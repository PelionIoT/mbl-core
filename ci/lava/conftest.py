...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        action="store",
        choices=["wks_bootloader1", "wks_bootloader2", "kernel"],
    )
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "pelion or mblcli")
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if (
            update_method.replace("-", "_") not in markers
            and not len(markers) == 0
        ):
...
# Boot component update fixtures
@pytest.fixture
